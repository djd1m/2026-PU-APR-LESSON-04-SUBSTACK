...
**Context:** SubStack RU — Russian Substack clone. Email via Resend. Stack: Next.js + NestJS + PostgreSQL + Redis + Docker on VPS Russia.
...
| validator-architecture | Architecture.md | Consistency with project Architecture (Next.js/NestJS/PostgreSQL/Redis) |
...
- Stack-specific: use TypeScript throughout, follow NestJS module structure, Next.js App Router conventions
- Email delivery: use Resend SDK, never mock email sending in production paths
...
| architecture | Integration | Consistency with NestJS/Next.js project architecture |
| security | Security surface | Vulnerabilities, input validation, Resend API key handling |
| performance | Hot paths | Bottlenecks, Redis cache usage, PostgreSQL query efficiency |
| testing | Test coverage | Edge cases, missing tests, email delivery scenarios |
