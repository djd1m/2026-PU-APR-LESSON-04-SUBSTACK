...
export default function EditorPage() {
  ...
  const getPayload = useCallback((): ArticlePayload => ({
    title,
    content: editor?.getHTML() ?? '',
    visibility,
    ...(scheduledAt ? { scheduledAt } : {}),
  }), [title, editor, visibility, scheduledAt])

  const handleSaveDraft = async () => {
    setSaving(true)
    setSaveStatus('idle')
    try {
      await api.post('/author/articles', { ...getPayload(), status: 'draft' })
      setSaveStatus('saved')
  ...
  const handlePublish = async (action: PublishAction) => {
    ...
    setPublishing(true)
    try {
      await api.post('/author/articles', {
        ...getPayload(),
        status: action === 'draft' ? 'draft' : action === 'schedule' ? 'scheduled' : 'published',
      })
      // Success: in real app, redirect to article or dashboard
    } catch {
      // Handle error
    } finally {
      setPublishing(false)
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          {saveStatus === 'error' && (
            <span className="text-xs text-red-400">Ошибка сохранения</span>
          )}
        </div>
